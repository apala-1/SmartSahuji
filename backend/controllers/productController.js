...
exports.addProduct = async (req, res) => {
  try {
    ...
    if (item_type === "Sale" && inventoryItem) {
      if (!price || !sale_type)
      ...
      if (inventoryItem.currentStock < qty)
        return res
          .status(400)
          .json({ error: `Only ${inventoryItem.currentStock} items available` });
    }

    if (item_type === "Purchase" && !cost) {
...
exports.deleteProduct = async (req, res) => {
  try {
    const { id } = req.params;
    const deleted = await Product.findOneAndDelete({ _id: id, user: req.user.id });
  ...
  try {
    ...
    if (req.file.mimetype === "text/csv") {
      ...
      products = rows.map((row) => ({
        ...
        sale_type: row.item_type === "Sale" ? row.sale_type || "Retail" : null,
